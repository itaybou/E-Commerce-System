...
namespace ECommerceSystemAcceptanceTests.store_owner_requirments
{
    ...
    internal class removeOwnerTests : StoreOwnerTests
    {
        ...
        public void removeOwnerRec()
        {
            ...
            Assert.IsFalse(_bridge.assignManager(_newManager, _storeName), "_newOwner2 didn't remove");
            Assert.IsFalse(_bridge.removeManager(_newManager, _storeName), "_newOwner2 didn't remove");
            _bridge.logout();

            //check permissions _newOwner1
            _bridge.login(_newOwner1, _pswd);
            Assert.IsFalse(_bridge.assignManager(_newManager, _storeName), "_newOwner1 didn't remove");
            Assert.IsFalse(_bridge.removeManager(_newManager, _storeName), "_newOwner1 didn't remove");
            _bridge.logout();
        }
