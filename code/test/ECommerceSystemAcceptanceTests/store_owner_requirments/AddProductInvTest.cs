...
namespace ECommerceSystemAcceptanceTests.store_owner_requirments
{
    ...
    class AddProductInvTest : StoreOwnerTests
    {
        ...
        public void addProductFail()
        {
            ...
            Assert.AreEqual(-1, _bridge.addProductInv(_storeName, _productName, _description, _discontType, _discountPercentage, _purchaseType, _price, _quantity, _category, _keywords), "guest add product inventory successed");

            //Regular user try to add productInv
            _bridge.login(_userName, _pswd);
            Assert.AreEqual(-1, _bridge.addProductInv(_storeName, _productName, _description, _discontType, _discountPercentage, _purchaseType, _price, _quantity, _category, _keywords), "regulat user add product inventory successed");
            _bridge.logout();

            //not permited manager try to add productInv
            _bridge.login(_managerUserName, _pswd);
            Assert.AreEqual(-1, _bridge.addProductInv(_storeName, _productName, _description, _discontType, _discountPercentage, _purchaseType, _price, _quantity, _category, _keywords), "not permited manager add product inventory successed");
            _bridge.logout();
