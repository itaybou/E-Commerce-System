...
namespace ECommerceSystemAcceptanceTests.store_owner_requirments
{
    ...
    class StorePurchaseHistoryTest : StoreOwnerTests
    {
        ...
        public new void oneTimeSetup()
        {
            ...
            _bridge.purchaseProduct((_storeName, (_productID, 1)), _fname, _lname, 123456789, "1234123412341234", DateTime.Today, 123, "address");
            _bridge.logout();
        }
        ...
        public void storePurchaseHistorySuccess()
        {
            ...
            //owner
            _bridge.login(_ownerUserName, _pswd);
            Assert.AreEqual(expectedHistory, _bridge.purchaseHistory(_storeName), "fail to return purchase history of store");
            _bridge.logout();
        ...
        {
            _bridge.login(_ownerUserName, _pswd);
            Assert.Null(_bridge.purchaseHistory("not exist"), "purchase history of not exist store successed");
            _bridge.logout();


            Assert.Null(_bridge.purchaseHistory(_storeName), "purchase history with guest successed");
            _bridge.login(_userName, _pswd);
            Assert.Null(_bridge.purchaseHistory(_storeName), "purchase history with regular user successed");
            _bridge.logout();
        }
