...
namespace ECommerceSystem.DomainLayer.UserManagement.Tests
{
    ...
    public class UsersManagementTests
    {
        ...
        public void setUpFixture()
        {
            ...
            {
                {new ProductInventory(null,null,20.0,Category.CELLPHONES,new List<string>(),Guid.NewGuid(),"","store1")}
            };
            store2.Inventory.Products = new List<ProductInventory>()
            {
                {new ProductInventory(null,null,20.0,Category.CELLPHONES,new List<string>(),Guid.NewGuid(), "", "store2")}
            };
            ...
            _storeManagement.getAllStores().Add(store);

        

            _storeManagement.getAllStores().Add(store2);
        ...
        public void addProductToCartTest()
        {
            ...
            Assert.AreEqual(cart.StoreCarts.First().ProductQuantities[product.Id].Item2, 1);        // Test product quantity added is one
            ...
            Assert.AreEqual(cart.StoreCarts.First().ProductQuantities[product.Id].Item2, 3);        // Test adding same product from same store, increases accumulated qunatity
            ...
            Assert.AreEqual(cart.StoreCarts.First().ProductQuantities[product2.Id].Item2, 1);       // Test product quantity added is one
            ...
            Assert.AreEqual(cart.StoreCarts.ElementAt(1).ProductQuantities[product2.Id].Item2, 1);  // Test new store cart contains the new product with quantity one
        }
        ...
        public void changeProductQuantityTest()
        {
            ...
            var cart = _userManagement.getUserCart(_userManagement.getUserByName(uname));
            Assert.AreEqual(cart.StoreCarts.First().ProductQuantities.First().Value, 8);   // Test product quantity changed to 8 (available amount)
