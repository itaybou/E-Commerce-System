...
namespace ECommerceSystem.DomainLayer.SystemManagement.Tests
{
    ...
    public class SearchAndFilterTests
    {
        ...
        public void setUpFixture()
        {
            ...
            {
                { ProductInventory.Create("Dell XPS 15inch", "Laptop", discount, purchasePolicy, 1600.0, 30, Category.ELECTRONICS, new List<string>() { "Laptop", "Computer", "PC"}, Guid.NewGuid() , Guid.NewGuid())},
                { ProductInventory.Create("Sony DSLR Camera", "Best camera", discount, purchasePolicy, 500.0, 20, Category.CAMERASPHOTOS, new List<string>() { "Camera", "DSLR", "Sony"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Google Pixel 4", "New google phone", discount, purchasePolicy, 700.0, 10, Category.CELLPHONES, new List<string>() { "Google", "Pixel", "Phone"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Google Pixel 4", "New google phone", discount, purchasePolicy, 700.0, 10, Category.ELECTRONICS, new List<string>() { "Google", "Pixel", "Phone"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Western Digital 4TB HardDrive", "4TB Harddrive", discount, purchasePolicy, 100.0, 100, Category.ELECTRONICS, new List<string>() { "WD", "Harddrive"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Dyson V11", "Vaccum cleaner", discount, purchasePolicy, 300.0, 5, Category.HOMEGARDEN, new List<string>() { "Dyson", "Vaccum"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Logitech MX Master 3", "Computer mice", discount, purchasePolicy, 110.0, 300, Category.ELECTRONICS, new List<string>() { "Logitech", "Mice"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Linkin Park - Meteora", "Nu-Metal music album", discount, purchasePolicy, 20.0, 2, Category.MUSIC, new List<string>() { "Music", "LP", "Nu-Metal"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("Windows 10 Home", "Microsoft Operatin system", discount, purchasePolicy, 25.0, 5000, Category.SOFTWARE, new List<string>() { "OS", "Microsoft", "Windows"}, Guid.NewGuid(), Guid.NewGuid())},
                { ProductInventory.Create("iPhone 11 XL", "Apple new smartphone", discount, purchasePolicy, 800.0, 30, Category.CELLPHONES, new List<string>() { "Apple", "Smartphone", "Phone"}, Guid.NewGuid(), Guid.NewGuid())},
            };
